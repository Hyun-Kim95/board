	...
	<div class="bg-white shadow-md rounded container mx-auto p-8 mt-8">
		<div class="w-full">
			<div class="flex items-center mt-4">
				<a href="../delArticle/list?boardId=${board.id}" class="px-2 py-1 bg-gray-600 text-gray-100 font-bold rounded hover:bg-gray-500">삭제된 게시물 목록</a>
				<div class="flex-grow"></div>
				<a onclick="if ( !confirm('복구하시겠습니까?') ) return false;" href="restore?id=${article.id}"
					class="btn-primary bg-blue-500 hover:bg-blue-dark text-white font-bold mx-1 py-1 px-2 rounded">복구</a>
				<a onclick="if ( !confirm('완전삭제하시겠습니까?') ) return false;"
					href="deleteCompletely?id=${article.id}"
					class="btn-primary bg-blue-500 hover:bg-blue-dark text-white font-bold py-1 px-2 rounded">완전삭제</a>
			</div>
			<div class="flex flex-row mt-2 py-3">
				<div class="flex flex-col mb-2 ml-4 mt-1">
					<div class="text-gray-600 font-semibold">작성자 : ${article.extra__writer}</div>
					<div class="flex w-full mt-1">
						<div class="text-gray-400 font-thin text-xs">${article.delDate}</div>
					</div>
				</div>
			</div>
